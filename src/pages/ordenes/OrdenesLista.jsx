          ...
          filtered.length > 0 ? (
            <div className="table-wrapper">
              <table className="table">
                <thead>
                  <tr><th>No.</th><th>Cliente</th><th>Vehículo</th><th>Placa</th><th>Estado</th><th>Total</th><th></th></tr>
                </thead>
                <tbody>
                  {filtered.map(o => (
                    <tr key={o.id}>
                      ...
                      <td>{o.vehiculoData?.marca} {o.vehiculoData?.linea} {o.vehiculoData?.modelo}</td>
                      <td>{o.vehiculoData?.placa || <span className="text-muted">—</span>}</td>
                      <td><span className={`badge ${STATUS_BADGE[o.estado] || 'badge-muted'}`}>{STATUS_LABEL[o.estado] || o.estado}</span></td>
                      <td><span style={{ fontWeight: 700, color: 'var(--color-accent)' }}>Q{Number(o.total || 0).toFixed(2)}</span></td>
                      <td><Link to={`/ordenes/${o.id}`} className="btn btn-secondary btn-sm">Ver</Link></td>
                    </tr>
                  ))}
                </tbody>
              </table>
